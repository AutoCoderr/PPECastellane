...
<table>
	<tr>
		<td><input type='button' value='Boite de reception' onclick='listmp()'><br>
			<input type='button' value='Messages envoyés' onclick='listms()'></td>
		<td id='display'></td>
	</tr>
</table>
<br>
...
function listmp() {
	$("#display").empty();
	if (MPs.length == 0) {
		$("#display").append("<font color='orange' size='4'>Aucun message</font>");
	} else {
		var html = "Boite de reception:";
		html += "<table>";
...
function listms() {
	$("#display").empty();
	if (MSs.length == 0) {
		$("#display").append("<font color='orange' size='4'>Aucun message envoyé</font>");
	} else {
		var html = "Message envoyés";
		html += "<table>";
...
function deleteFromAlreadyChoiced(id) {
	for (var i=0;i<comptesAlreadyChoiced.length;i++) {
		if (id == comptesAlreadyChoiced[i]) {
			if (i == 0) {
				comptesAlreadyChoiced.splice(i,i+1);
			} else {
				comptesAlreadyChoiced.splice(i,i);
			}
		}
	}
}
