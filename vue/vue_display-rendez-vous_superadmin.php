...
function display_rendez_vous(date) {
	...
	for (var i=0;i<rendez_vous[date].length;i++) {
		...
		if (rendez_vous[date][i].type == "Planning") {
			...
			$("#display_rdv").append(" avec "+rendez_vous[date][i].clientprenom+" "+rendez_vous[date][i].clientnom+" (Client)<br>");
			...
			if (rendez_vous[date][i].validated == "yes") {
				$("#display_rdv").append(" (Validé par "+rendez_vous[date][i].moniteurprenom+" "+rendez_vous[date][i].moniteurnom+")");
			} else {
			...
			$("#display_rdv").append(" avec "+rendez_vous[date][i].clientprenom+" "+rendez_vous[date][i].clientnom+" <br>");
			if (rendez_vous[date][i].validated == "yes") {
				$("#display_rdv").append(" (Validé par "+rendez_vous[date][i].moniteurprenom+" "+rendez_vous[date][i].moniteurnom+")");
			} else {
				$("#display_rdv").append(" (Non validé)");
		...
		if (rendez_vous[date][i].validated == "yes") {
			...
			$("#display_rdv").append("<br>");
		}
		$("#display_rdv").append(remplace(annulerhtml,"this_rendezvous",i))
	}
}
</script>